    ...
    for (condition_type in names(conditions)) {
      ...
      if (condition_type == "between") {
        ...
        for (threshold in thresholds) {
          prob_var_name <- ifelse(threshold < 0 && condition_type == "lesser", sprintf("%s.lesserNeg%.2f", param, abs(threshold)), sprintf("%s.%s%.2f", param, condition_type, threshold))
          if (param %in% predictor_vars) {
            if (condition_type == "lesser") {
              model_string <- paste0(model_string, "  ", prob_var_name, " <- step(", threshold, " - beta[", which(predictor_vars == param), "])\n")
            } else {
              model_string <- paste0(model_string, "  ", prob_var_name, " <- step(beta[", which(predictor_vars == param), "] - ", threshold, ")\n")
            }
          } else {
            if (condition_type == "lesser") {
              model_string <- paste0(model_string, "  ", prob_var_name, " <- step(", threshold, " - ", param, ")\n")
            } else {
